...
const WEIGHTS = {
  ...
  send_valid:          20,  // POST /money/send      valid payload            — nominal write
  send_bad:            15,  // POST /money/send      invalid payload (Zod 400) — validation exercise
...
async function doSendValid(state) {
  if (state.active.length < 2) {
    return { ok: false, status: 0, ms: 0, endpoint: "POST /money/send (valid)", skipped: true };
  }
  ...
  if (amount <= 0) return { ok: false, status: 0, ms: 0, endpoint: "POST /money/send (valid)", skipped: true };
  ...
  await res.text();
  return { ok: res.ok, status: res.status, ms: now() - t0, endpoint: "POST /money/send (valid)" };
}
...
async function doSendBad() {
  ...
  await res.text();
  const ok = res.status === 400;  // expect Zod 400
  return { ok, status: res.status, ms: now() - t0, endpoint: "POST /money/send (bad)" };
}
